...
export default async function TenantOverviewPage({ params }: { params: { tenantId: string } }) {
  ...
  const [latestMetric] = await db.select().from(metrics)
    .where(eq(metrics.tenantId, params.tenantId))
  ...
  const componentStatuses = [
    { name: "Kafka", key: "kafka", status: totalLag < 100000 ? "running" : "degraded" as const, detail: `lag: ${totalLag.toLocaleString()}` },
    { name: "Flink", key: "flink", status: flinkStatus.length > 0 && runningJobs === flinkStatus.length ? "running" : "unknown" as const, detail: `${runningJobs}/${flinkStatus.length} jobs` },
    { name: "OpenSearch", key: "opensearch", status: osHealth.status === "green" ? "running" : osHealth.status === "yellow" ? "degraded" : "unknown" as const, detail: osHealth.status },
    { name: "Ticketing Agent", key: "ticketingAgent", status: "running" as const },
    { name: "ML Engine", key: "mlEngine", status: "running" as const },
  ];

  const overallHealth = componentStatuses.some((c) => c.status === "down") ? "critical"
    : componentStatuses.some((c) => c.status === "degraded") ? "degraded" : "healthy";
